...
def my_callback(msg: String, navigator: BasicNavigator):
    ...
    if msg.data == "A":
        ...
        navigator.warn(f"未知目标点: {msg.data}")

    if goal_pose is not None:
        new_goal_pose = goal_pose
        new_goal_pose_str = f'新目标点姿态: ({new_goal_pose.pose.position.x}, {new_goal_pose.pose.position.y}, {new_goal_pose.pose.orientation.w})'
        navigator.info(new_goal_pose_str)
    else:
        navigator.warn("未设置新的目标点姿态")
